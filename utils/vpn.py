...
def diagnose_openvpn_failure(log_tail: list[str]) -> tuple[int, str]:
    ...
    if (
        "cannot allocate tun" in joined_log
        or "cannot open tun/tap dev" in joined_log
        or "cannot ioctl" in joined_log
        or "cannot allocate tun/tap dev" in joined_log
        or "dev/net/tun" in joined_log
    ):
        return 2009, "[ERR_OVPN_TUN_NOT_AVAILABLE] 无法创建虚拟网卡 (TUN 设备)。原因: ① 系统缺少 tun 内核模块；② VPS/容器未开启 TUN/TAP；③ `/dev/net/tun` 不存在或权限不足；④ Docker/LXC 未授予 NET_ADMIN 能力。请先在 VPS 面板启用 TUN，容器环境需添加 `--cap-add=NET_ADMIN` 和 `--device=/dev/net/tun`。"
    ...
    if "permission denied" in joined_log or "operation not permitted" in joined_log or "must be root" in joined_log or "root privileges" in joined_log:
        return 2002, "[ERR_OVPN_PERMISSION_DENIED] OpenVPN 权限不足。原因: 当前进程没有 root 权限或缺少 NET_ADMIN/NET_RAW 能力，无法创建网卡、设置路由或绑定设备。请使用 root 运行服务，容器环境需授予网络管理权限。"

    if "options error" in joined_log:
        if "unrecognized option" in joined_log or "unknown option" in joined_log:
            return 2008, "[ERR_OVPN_OPTION_UNSUPPORTED] OpenVPN 参数不兼容。原因: 当前系统的 OpenVPN 版本过旧或编译选项不完整，不支持配置/启动参数中的某些选项。请升级 OpenVPN 或检查节点配置。"
        return 2007, "[ERR_OVPN_ROUTE_NOPULL] 获取/解析 PUSH 配置参数冲突。原因: 某些推送选项在当前版本的客户端或配置环境中不可用。"

    if "exiting due to fatal error" in joined_log:
        return 2011, "[ERR_OVPN_FATAL] OpenVPN 发生致命错误并退出。原因: 可能是配置文件、证书、权限或系统网络环境冲突导致核心无法继续运行，请查看日志尾部的原始错误。"
